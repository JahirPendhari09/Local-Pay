...
const DashBoard =()=>{
    ...
    <Grid templateColumns={['1fr', '1fr 1fr', '1fr 1fr 1fr 1fr']} gap="40px" p={4} margin="50px" padding={4} color="cyan.700" fontSize="25px" >
      ...
      {history.map((item,i)=>{
        return <HStack id={item.id} justifyContent="space-around"w="60%" margin="auto" border="1px" borderRadius="20px"  p={4} fontSize="20px"bg="bisque" marginBottom="20px" gap={4}>
            <Text border="1px solid black" marginLeft={0} borderRadius="50%" w="60px" h="60px" align="center" paddingTop="15px" bg={bgColor[i%bgColor.length]}>{item.firstName.charAt(0).toUpperCase()+item.lastName.charAt(0).toUpperCase()}</Text>
            <VStack>
